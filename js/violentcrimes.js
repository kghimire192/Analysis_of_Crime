...
function drawGraphs() {
    plotEachGraph('violentCrime', 
                  'SELECT A,B',
                  totalViolentCrimesHandler);

    plotEachGraph('violentCrime', 
                  'SELECT D, G',
                  violentCrimeRateGrowthHandler);
...
//draws pie chart in this case to show the violent crimes distribution of the total violent crimes in the US from 2010 to 2017
function totalViolentCrimesHandler(response) {
    var data = response.getDataTable(); //putting data in a data table
    var options = {
            title: 'Total Violent Crimes (2010 - 2017)',	
    ...
    var chart = new google.visualization.PieChart(
                document.getElementById('total_violent_crimes'));
...
function violentCrimeRateGrowthHandler(response) {
	...
	var chart = new google.visualization.GeoChart(
				document.getElementById('violent_Growth_percent'));
...
function timeSeriesGrowthHandler(response) {
	var data = response.getDataTable();
	var options = {
		        title: 'Violent Crime Rate from 2011 to 2017',
				vAxis: {title: 'Violent Crime Rate'},
				hAxis: {title: 'Year'},	
				backgroundColor: '#E0DFDE'
	};
...
document.addEventListener("DOMContentLoaded", () => {console.log("Success")
...
const drawBarGraph = () => {
	...
	.html(function(d){
		...
		text += "<strong>Violent Crime Rate: </strong><span style='color:#00CCFF'>" + d3.format("0.2f")(d.ViolentCrimeRate) + "</span><br>";
		return text;
	})
	...
	function renderBarGraph(file_name) {
		d3.csv(file_name).then(function(given_data) {
			...
			given_data = given_data.sort((a,b) => b.ViolentCrimeRate - a.ViolentCrimeRate)


			// given_data = given_data.sort(function(a,b){
			// 	console.log(d3)
			// 	return d3.descending(a.ViolentCrimeRate, b.ViolentCrimeRate)
			// })
			
			
			given_data.forEach(function(d){
				d.ViolentCrimeRate = +d.ViolentCrimeRate; //casting into integer
			});
			...
			.domain([0, d3.max(given_data, function(d){ return d.ViolentCrimeRate}) + 20]) 
			.range([height,0]);
			...
			.attr("y", function(d){ return y(d.ViolentCrimeRate) })
			.attr("width", x.bandwidth())
			.attr("height", function(d){ return height-y(d.ViolentCrimeRate) })
			.attr("fill", "#1e5878")
			//.attr("fill", "#69a7ac")
